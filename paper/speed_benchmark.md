...
| SWAMPE-JAX (my_swamp)  | CPU (1 core)           | 25.220 s     | 32.4x |
| SWAMPE-JAX (my_swamp)  | GPU (NVIDIA A100-40GB) | 1.1535 s     | 708.6x |

- CPU timings: Apple M3 Pro, single core. GPU timing: Colab `NVIDIA A100-SXM4-40GB`, float64.
- **my_swamp GPU vs my_swamp CPU** (same code, cross-device): **21.9x**.
...
- **Batched-vs-SWAMPE speedup, efficient batch (N=32): 6,330x.** (817.353 s / 0.1291245 s.)
  Peak batch (N=64): 6,632x.
- Provenance: `paper/benchmark_data/gpu_vmap_sweep_10day.json`, generated by
  `paper/scripts/swampe_gpu_vmap_test.ipynb` (Colab, `NVIDIA A100-SXM4-40GB`, float64, 10-day
  runs, JIT compile excluded via warmup, `block_until_ready()`). A CLI-equivalent script,
  `paper/scripts/swampe_gpu_vmap_test.py`, exists for non-Colab GPU machines and produces the
  same schema, but was not the source of the numbers quoted here -- the notebook was.
...
These are the exact numbers that should appear in `paper.tex`'s "Speed (CPU and GPU)"
subsection. If you regenerate any of the data above, update both this file and the paper
...
(These are the four numeric substitutions actually made in `paper.tex` on 2026-06-30; the
surrounding prose was deliberately left unchanged -- this file carries the extra caveats
and methodology detail that did not go into the paper text itself.)
